...
export function buildActionPlanPromptFragment(input: { manifest?: DomainManifest; currentDate?: string; timezone?: string; compact?: boolean } = {}) {
  ...
  return [
    ...
    "Maiores ou menores gastos, despesas, receitas ou lancamentos financeiros SEM pedido explicito de agrupar por categoria ranqueiam TRANSACOES INDIVIDUAIS, nao entidades: domain=financeiro, filtro tipo, SEM groupBy e SEM aggregations, apenas orderBy valor desc (maiores) ou asc (menores) e limit conforme pedido (ou 10 quando o usuario nao disser quantos). Categoria em financeiro quase sempre repete o mesmo valor generico (por exemplo 'Despesa'), entao agrupar por ela ou por descricao esconde os lancamentos e produz poucos grupos artificiais. Use groupBy categoria somente quando o usuario pedir explicitamente 'por categoria' ou equivalente.",
